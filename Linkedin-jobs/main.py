"""
Pipeline: LinkedIn job search (by criteria) -> scrape job pages -> write resume-tailoring docs.

Job source: config/linkedin_search.json (keywords, locations, date posted, etc.)
...
from src.config import OUTPUT_DIR, load_linkedin_search_config
...
def main(
    ...
    print("Fetching job URLs from LinkedIn search (config/linkedin_search.json)...")
    ...
    if not urls:
        print("No job URLs found. Check config/linkedin_search.json.")
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="LinkedIn search -> scrape -> resume docs")
