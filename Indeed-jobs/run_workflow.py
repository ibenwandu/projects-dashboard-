"""
Fully automated workflow: Indeed search → scrape → score with AI → report → (user approves) → tailored resumes.
...
def run_score_and_report(
    ...
    # Exclude jobs that have already been included in a previous report
    ...
    # Record these job IDs so they are excluded from future reports
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Indeed-jobs workflow: fetch → score → report → resumes")
    ...
    parser.add_argument("--include-reported", action="store_true",
                        help="Include jobs that were already in a previous report (default: exclude them)")
