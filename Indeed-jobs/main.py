"""
Pipeline: Indeed search (by criteria) → scrape job pages → write resume-tailoring docs.

Job source: config/indeed_search.json (keywords analyst/manager, Toronto/Remote, salary>80k, last 24h, etc.)
...
from src.indeed_search import get_search_result_job_urls
from src.indeed_scraper import scrape_jobs_with_browser, is_blocked_job
from src.indeed_search import filter_jobs_by_criteria
...
def main(
    ...
    print("Fetching job URLs from Indeed search (criteria in config/indeed_search.json)...")
    ...
    if not urls:
        ...
        return
    print("Found", len(urls), "job URL(s). Scraping details (using Chrome + anti-detection)...")
    ...
    if not jobs:
        print("No jobs could be scraped. Try fewer URLs (e.g. --max-jobs 5) or add --headless to run in background.")
        return
    if not no_filter_job_type and config.get("job_types"):
        jobs = filter_jobs_by_criteria(jobs, config=config)
        print(f"After filtering by job type: {len(jobs)} job(s).")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Indeed search → scrape → resume docs")
