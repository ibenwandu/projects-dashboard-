...
class TradeExecutor:
    ...
    def _request_with_retry(self, r, label: str = "request"):
        """Call client.request(r) with retries on connection errors."""
        last_e = None
    ...
    def open_trade(self, trade: ManagedTrade) -> Optional[str]:
        ...
        try:
            ...
            # consol-recommend4 Phase 1.1: JPY limit/stop order price scale and sanity check
            # JPY pairs (e.g. USD_JPY) trade in 15x.xxx; entry must not be wrongly scaled (e.g. 1.56 instead of 156.xxx)
            entry_price = trade.entry_price
            pair_upper = (trade.pair or "").upper()
            if "JPY" in pair_upper and order_type in ("LIMIT", "STOP"):
                if entry_price < 10 and entry_price > 0:
                    # TP/SL are typically in correct scale (e.g. 157.5); if we have TP in right scale, correct entry
                    if trade.take_profit and trade.take_profit > 10:
                        entry_price = entry_price * 100.0
                        self.logger.info(
                            f"JPY price scale corrected for {trade.pair}: entry {trade.entry_price} -> {entry_price} (TP scale)"
                        )
                    else:
                        self._last_reject_reason = "validation_failed"
                        self.logger.error(
                            f"JPY pair {trade.pair}: entry price {trade.entry_price} is below 10 (invalid scale). "
                            "Refusing to send order. Fix upstream entry to use same scale as TP/SL (e.g. 156.xxx)."
                        )
                        return None
                elif entry_price <= 0:
                    self._last_reject_reason = "validation_failed"
    ...
    def update_stop_loss(self, trade_id: str, new_sl: float, pair: str) -> bool:
        """Update stop loss to fixed price"""
        try:
            # Round stop loss to OANDA precision
    ...
    def convert_to_trailing_stop(self, trade_id: str, distance_pips: float, pair: str) -> bool:
        """Convert fixed SL to trailing stop"""
        try:
            ...
            try:
                ...
                # OANDA API error - request failed
                error_msg = getattr(e, 'msg', str(e))
                self.logger.error(f"❌ OANDA API error converting to trailing stop for trade {trade_id}: {error_msg}")
                return False
...
class PositionManager:
            ...
            if os.path.exists(self.state_file):
                with open(self.state_file, 'r') as f:
                    ...
                    for trade_data in data.get('trades', []):
                        ...
                        if trade.trade_id and trade.state in [TradeState.OPEN, TradeState.AT_BREAKEVEN, TradeState.TRAILING]:
    ...
    def can_open_new_trade(self) -> Tuple[bool, str]:
        ...
        Counts BOTH in-memory active_trades AND actual OANDA positions/orders
        This ensures the limit is enforced even after service restarts
        """
        # Count in-memory active trades (includes PENDING orders)
        in_memory_count = len(self.active_trades)
        
        # Also check OANDA directly to get actual count (critical after restarts)
        oanda_count = 0
        try:
            ...
            # Also get pending orders from OANDA (they count toward the limit)
            try:
                from oandapyV20.endpoints.orders import PendingOrders
                r = PendingOrders(accountID=self.executor.account_id)
                self.executor.request(r, "pending_orders")
                pending_orders = r.response.get('orders', [])
                oanda_count += len(pending_orders)
            except Exception as e:
                # If we can't get pending orders, just use trades count
                self.logger.debug(f"Could not get pending orders from OANDA: {e}")
            
        except Exception as e:
            # If OANDA check fails, fall back to in-memory count
            self.logger.debug(f"Could not check OANDA for trade count: {e}")
            oanda_count = in_memory_count
        
        # Use the maximum of both counts to ensure we don't exceed limit
        # This handles cases where OANDA has trades we haven't synced yet
        actual_count = max(in_memory_count, oanda_count)
        
        if actual_count >= self.config.max_open_trades:
            return False, f"Max trades limit reached ({actual_count}/{self.config.max_open_trades}) - in-memory: {in_memory_count}, OANDA: {oanda_count}"
        
        # TODO: Add daily loss check
        # TODO: Add total exposure check
    ...
    def has_existing_position(self, pair: str, direction: str) -> bool:
        ...
        for trade in self.active_trades.values():
            ...
            if trade_pair == normalized_pair and trade_direction_normalized == direction_normalized:
                # Check if trade is still actually open in OANDA (not just in memory)
                if trade.state == TradeState.PENDING:
                    # For pending orders, check if order still exists
                    if trade.trade_id and self.executor.check_order_exists(trade.trade_id):
                        return True
                else:
                    # For open trades, assume they're still open (we sync periodically)
                    return True
    ...
    def open_trade(self, opportunity: Dict, market_state: Dict, record_run: bool = True) -> Optional[ManagedTrade]:
        ...
        if self.has_existing_position(pair, direction):
            ...
            self.logger.warning(
                f"🚫 BLOCKED: Attempted to open duplicate trade - {pair} {direction} "
                f"already exists (final safety check)"
            )
            return None
        ...
        if directive.action == "WAIT_SIGNAL":
            ...
            if order_or_trade_id:
                # Record execution (CRITICAL - prevents repeated execution via max_runs)
                # Skip when record_run=False (e.g. replace pending - same run as cancelled order)
                if record_run:
                    opp_id = get_stable_opportunity_id(opportunity)
    ...
    def _create_trade_from_opportunity(self, opp: Dict, market_state: Dict) -> ManagedTrade:
        ...
        # Use recommended entry price (opp['entry'] should be the recommended entry)
        ...
        # Fisher uses 'take_profit', LLM uses 'exit'
        stop_loss = opp.get('stop_loss')
        take_profit = opp.get('take_profit') or opp.get('exit')
    ...
    def sync_with_oanda(self, market_state=None):
        ...
        try:
            ...
            # Add new trades from OANDA
            for trade_id, oanda_trade in trades_to_add:
                try:
                    # Optional orphan detection: log WARNING once per (pair, direction) per window (suggestions cursor3 §5.3)
                    ...
                    # Convert OANDA trade to ManagedTrade
                    managed_trade = self._create_trade_from_oanda(oanda_trade)
                    if managed_trade:
                        self.active_trades[trade_id] = managed_trade
                        self.logger.info(
    ...
    def _check_be_transition(self, trade: ManagedTrade, current_price: float):
        """Check if trade should transition from fixed SL to trailing"""
        
        if trade.state == TradeState.OPEN:
            # Check if price reached breakeven point
            pip_size = 0.01 if "JPY" in trade.pair else 0.0001
            be_threshold = trade.entry_price
            is_long = (trade.direction or "").upper() in ("LONG", "BUY")
            if is_long:
                at_be = current_price >= be_threshold
            else:
                at_be = current_price <= be_threshold
            ...
            if at_be:
                ...
                if self.executor.update_stop_loss(trade.trade_id, trade.entry_price, trade.pair):
                    ...
                    self.logger.info(
                        f"🎯 Trade {trade.trade_id} at breakeven - SL moved to entry"
                    )
    ...
    def _check_ai_trailing_conversion(self, trade: ManagedTrade, current_price: float, market_state: Dict):
        ...
        2. When trade moves in profit (reaches breakeven or better), convert to trailing stop
        3. Trailing distance is calculated from ATR and volatility regime
        """
        pip_size = 0.01 if "JPY" in trade.pair else 0.0001
        
        if trade.state == TradeState.OPEN:
            # Check if price reached breakeven point (or moved in profit)
            is_long = (trade.direction or "").upper() in ("LONG", "BUY")
            if is_long:
                at_be_or_profit = current_price >= trade.entry_price
            else:
                at_be_or_profit = current_price <= trade.entry_price
            
            if at_be_or_profit:
                # Trade is at breakeven or in profit - convert to trailing stop
                # Recalculate trailing distance in case volatility regime changed
                regime = market_state.get('regime', 'NORMAL')
                if regime == 'HIGH_VOL':
                ...
                # Update stored trailing distance
                trade.trailing_distance = trailing_pips
...
class RiskController:
    ...
    def can_take_loss(self, potential_loss: float) -> Tuple[bool, str]:
        """Check if we can afford potential loss"""
    ...
    def record_trade_close(self, realized_pnl: float):
        """Record trade closure for daily tracking"""
        self.check_daily_reset()
