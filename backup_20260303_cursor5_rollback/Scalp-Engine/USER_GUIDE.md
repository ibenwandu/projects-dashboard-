...
#### 5. UI Database (scalping_rl.db) and Streamlit Session (consol-recommend2 Phase 1.7)
...
#### 16. OANDA app log and transaction history (consol-recommend4 Phase 3.5)

The **OANDA app log** (Config API `/logs/oanda`) may be empty when there is no recent activity. For broker-side checks use **OANDA transaction history** (e.g. `oanda_transactions_*.json` from backup or OANDA API).

---

#### 17. Trailing SL verification and trade-close audit log (consol-recommend4 Phase 1.4)

**Trailing SL**: The engine applies trailing stop logic in the **main monitoring loop** for each open trade. For **BE_TO_TRAILING**: when price reaches breakeven, SL is moved to entry via `update_stop_loss`; then the trade is converted to a **trailing stop** via `convert_to_trailing_stop` (OANDA distance-based). For **longs**, SL moves up (breakeven then trails below price); for **shorts**, SL moves down. For **ATR_TRAILING**, conversion to trailing uses ATR-based distance. You can verify in OANDA that open positions show a trailing stop, or in logs for "converted to trailing stop" / "SL moved to entry".
