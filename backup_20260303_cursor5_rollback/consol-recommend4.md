...
**Status:** Partially implemented (Mar 2, 2026). Phase 1.1 (JPY price), 1.4 (trailing SL verify + close log doc), Phase 2.1 (consensus display), 2.3 (DeepSeek parser + doc), Phase 2.2/2.4/2.5/2.6 and Phase 3 docs in USER_GUIDE, Phase 3.1 (sync/orphan doc + optional WARNING log). Phase 0.3/1.2 verified in code (max_runs reset present). One change at a time; verify trades still open after each deploy.
