...
Cloudflare/bot blocks. Use for Indeed search and job detail pages.
"""
from __future__ import annotations

# Realistic viewport and locale for Canada
DEFAULT_VIEWPORT = {"width": 1920, "height": 1080}
DEFAULT_LOCALE = "en-CA"
DEFAULT_TIMEZONE = "America/Toronto"
# Chrome 120 on Windows (update periodically)
DEFAULT_USER_AGENT = (
...
def get_context_options(headless: bool = True) -> dict:
    ...
    Reduces (but does not eliminate) Cloudflare "Additional Verification" blocks.
    """
    return {
...
def get_stealth_init_script() -> str:
    """
    Inject script to hide common automation signals (e.g. navigator.webdriver).
    Run via context.add_init_script() before navigating.
    """
    return """
...
def launch_browser(playwright, headless: bool = True, use_chrome: bool = True):
    """
    Launch browser. Prefer installed Chrome (channel="chrome") over Chromium
    when use_chrome=True; Chrome is less often flagged by Cloudflare.
    """
    try:
        if use_chrome:
