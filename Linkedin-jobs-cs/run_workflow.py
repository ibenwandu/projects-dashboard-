"""
Fully automated workflow: LinkedIn search -> scrape -> score with AI -> report -> (user approves) -> tailored resumes.

Phases:
  fetch   - LinkedIn search (config/linkedin_search.json) -> scrape -> save jobs_cache.json + markdown in output/
...
from src.config import (
    ...
    REPORTS_DIR,
    TOP_N_JOBS,
    load_linkedin_search_config,
)
from src.linkedin_search import get_search_result_job_urls, filter_jobs_by_criteria
from src.linkedin_scraper import scrape_jobs_with_browser, is_blocked_job
...
def run_fetch(
    ...
    """Fetch from LinkedIn search, scrape, save cache and markdown. Returns list of JobDetails."""
    config = load_linkedin_search_config()
    print("Fetching job URLs from LinkedIn search...")
    ...
    if not urls:
        print("No LinkedIn job URLs found. Check config/linkedin_search.json. Run with --debug to see why.")
        return []
    if max_jobs:
        urls = urls[:max_jobs]
    print("Found", len(urls), "job URL(s). Scraping (Chrome + anti-detection, slower to reduce blocks)...")
    all_jobs = scrape_jobs_with_browser(urls, headless=headless, delay_between=3.0, delay_jitter=2.0, debug=debug)
    ...
    print("Saved cache to", JOBS_CACHE_PATH)
    return jobs
...
def run_score_and_report(
    ...
    if not skip_reported_filter:
        ...
        if excluded:
            print("Excluding", excluded, "job(s) already reported in a previous run.")
        if not jobs_new:
    ...
    print("Scoring", len(jobs), "job(s) with AI...")
    ...
    if not skip_reported_filter and scored:
        ...
        print("Recorded", len(jks), "job ID(s) in output/reported_job_ids.txt (excluded from future reports).")
...
def run_pdfs() -> list:
    ...
    if created:
        print("Created", len(created), "PDF(s) in output/resumes/")
    else:
...
def run_resumes() -> list:
    ...
    if written:
        ...
        print("Wrote", n_md, "tailored resume(s) (.md + .pdf) to output/resumes/")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Linkedin-jobs workflow: fetch -> score -> report -> resumes")
    parser.add_argument("--phase", choices=["fetch", "score", "report", "resumes", "pdf", "all"], default="all",
                        help="fetch=LinkedIn search+scrape; score+report=score and write report; resumes=tailored resumes + PDFs; pdf=convert .md to PDF only; all=fetch+score+report")
    ...
    parser.add_argument("--debug", action="store_true", help="Print debug info (search URL count, why job page blocked)")
    ...
    phase = args.phase
    if phase == "all":
        run_fetch(max_jobs=args.max_jobs, headless=args.headless, no_filter_job_type=args.no_filter_job_type, debug=args.debug)
