...
def _md_to_html(md: str, nl2br: bool = False) -> str:
    import markdown
    extensions = ["extra"]
    if nl2br:
...
def _preprocess_cover_letter_md(md: str) -> str:
    """Strip leading whitespace so header is not a code block; preserves **bold** and line structure with nl2br."""
    return "\n".join(line.strip() for line in md.split("\n"))


def _wrap_cover_letter_header(soup: BeautifulSoup) -> None:
    """Wrap first two top-level elements (sender + date) in .letter-header-right so they align left together."""
    top_level = [el for el in soup.children if hasattr(el, "name") and el.name]
    if len(top_level) < 2:
        return
    wrapper = soup.new_tag("div", attrs={"class": "letter-header-right"})
    for el in top_level[:2]:
        el.extract()
        wrapper.append(el)
...
def md_file_to_pdf(
    ...
    # Cover letter: strip leading whitespace (so header isn't code block) and preserve single newlines as <br>
    is_cover_letter = template_path and (
        "cover-letter" in str(template_path) or "cover_letter" in str(template_path)
    )
    if is_cover_letter:
        md = _preprocess_cover_letter_md(md)
    html_body = _md_to_html(md, nl2br=is_cover_letter)
    soup = BeautifulSoup(html_body, "html.parser")
    if is_cover_letter:
        _wrap_cover_letter_header(soup)
