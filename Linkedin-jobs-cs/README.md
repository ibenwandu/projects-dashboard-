...
Automated pipeline focused on **Customer Success, Customer Experience (CX), and affiliated roles**: **LinkedIn job search** → scrape job details → score with AI (Gemini or OpenAI) → report (top N matches) → tailored resumes (.md + .pdf) and job application links for approved jobs.

Mirrors [Linkedin-jobs](../Linkedin-jobs) with customer-success–oriented search keywords and scoring criteria (same structure; config tuned for CX, Customer Success Manager, Business Development, Data-Driven Strategist, etc.).
...
- `--include-reported` — Include jobs already in a previous report (default: exclude them).

**Already-reported filter:** Job IDs in `output/reported_job_ids.txt` are excluded from future score/report runs. Delete or edit that file to clear the list.
