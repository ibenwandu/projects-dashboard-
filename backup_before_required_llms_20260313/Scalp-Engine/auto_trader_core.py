...
class TradeConfig:
    ...
    be_min_pips: float = 40.0  # Research: move SL to breakeven when profit >= this (default 40)
    trailing_activation_min_pips: float = 70.0  # Research: activate trailing only when profit >= this (default 70)
...
class TradeExecutor:
    ...
    def close_trade(self, trade_id: str, reason: str = "Manual close") -> bool:
        """Close a trade"""
        self.logger.debug(f"🔧 close_trade() called: trade_id={trade_id}, reason={reason}")
        try:
            # Use OANDA v20 API pattern
...
class PositionManager:
    ...
    def cancel_all_pending_for_pair(self, pair: str, reason: str = "Cleanup: pair has open position - no pending allowed") -> int:
        """
        Cancel all pending orders on OANDA for the given pair.
        Returns the number of cancelled orders.
        
        This is used immediately after opening a MARKET trade to ensure there is
        never a window where the same pair has both an open position and one
        ...
        try:
            ...
            for o in pending_orders:
                inst_raw = (o.get('instrument') or '')
                inst = self.normalize_pair(inst_raw.replace('-', '_'))
                if inst != normalized_pair:
                    continue
                oid = str(o.get('id', ''))
                if not oid:
                    continue
                if self.executor.cancel_order(oid, reason):
                    self.active_trades.pop(oid, None)
                    cancelled += 1
            if cancelled > 0:
                self.logger.info(
                    f"🧹 Cancelled {cancelled} pending order(s) for {pair} "
                    f"after market open (no pending allowed while pair has open position)"
                )
            else:
                self.logger.debug(
                    f"No pending orders found to cancel for {pair} after market open"
                )
        except Exception as e:
            self.logger.warning(
                f"⚠️ Failed to cancel pending orders for {pair} after market open: {e}"
            )
        return cancelled
    ...
    def open_trade(self, opportunity: Dict, market_state: Dict, record_run: bool = True) -> Optional[ManagedTrade]:
        ...
        if directive.action == "WAIT_SIGNAL":
            ...
            if order_or_trade_id:
                ...
                self.active_trades.pop(pending_key, None)
                # After a successful MARKET open, immediately cancel any remaining pending
                # orders for this pair so we never have open + pending at the same time.
                if directive.order_type == 'MARKET':
                    self.cancel_all_pending_for_pair(
                        trade.pair,
                        reason="Cleanup after MARKET open: pair has open position - no pending allowed",
                    )
                if record_run:
