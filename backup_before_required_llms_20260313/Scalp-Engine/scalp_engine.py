...
class ScalpEngine:
    ...
    def __init__(self):
        ...
        # max_trades_limit log throttle: first per (pair, direction) per window at WARNING, rest at DEBUG (cursor7 Priority 2.3)
        self._max_trades_limit_last_logged: Dict[tuple, float] = {}
        self._max_trades_limit_window_seconds = 900  # 15 min
        ...
        # cursor6 §5.3: after cancelling stale order, record (pair, direction) -> (entry_price, timestamp) to avoid re-place same level
        self._stale_cancel_record: Dict[tuple, tuple] = {}  # (pair_norm, dir_norm) -> (entry_price, time.time())
        self._stale_cancel_cooldown_seconds = 600  # 10 min
        ...
        for attempt in range(max_retries):
            try:
                ...
                if 'required_llms' not in data and 'require_gemini' in data:
                    ...
                    del data['require_gemini']
                # Strip keys not in TradeConfig (cursor_works: do not pass unknown keys)
                _valid = {f.name for f in fields(TradeConfig)}
                ...
                # Defensive check: ensure attributes exist (in case called before __init__ completes)
                if not hasattr(self, '_config_loaded_once'):
                    self._config_loaded_once = False
                if not hasattr(self, '_last_config_snapshot'):
    ...
    def _auto_trader_mode_check(self):
        ...
        # Log active trades summary (cursor6 §5.1: display Open vs Pending to match max_trades semantics)
        if self.position_manager.active_trades:
            summary = self.position_manager.get_active_trades_summary()
            open_count = summary.get('open_count', summary['total_trades'])
            pending_count = summary.get('pending_count', 0)
            max_trades = self.config.max_open_trades
            self.logger.info(
                f"📈 Open: {open_count}/{max_trades}, Pending: {pending_count}, "
    ...
    def _check_new_opportunities(self, market_state: Dict):
        ...
        for opp in opportunities:
            try:
                ...
                # No pending when pair has open: skip any opportunity for a pair that already has an open position
                if self.position_manager.has_open_position_for_pair(pair):
                    self.logger.debug(
                        f"Skipped {pair} {direction} - pair already has an open position (no second open/pending allowed)"
                    )
                    continue
                ...
                if not can_open:
                    # Throttle: first per (pair, direction) per window at WARNING, rest at DEBUG (cursor7 Priority 2.3)
                    key = (pair, direction)
                    now_ts = time.time()
                    last_ts = self._max_trades_limit_last_logged.get(key, 0)
                    if now_ts - last_ts >= self._max_trades_limit_window_seconds:
                        self.logger.warning(f"⏭️ Skipped {pair} {direction} - {reason}")
                        self._max_trades_limit_last_logged[key] = now_ts
                    else:
                        self.logger.debug(f"⏭️ Skipped {pair} {direction} - {reason} (throttled)")
                ...
                if execution_mode == ExecutionMode.HYBRID:
                    ...
                    self.logger.info(
                        ...
                        f"AND monitoring MACD crossover - whichever triggers first, Consensus: {consensus}/{opp.get('available_llm_count', 3)}"
                    )
                    ...
                    self.logger.info(
                        ...
                        f"(MACD signal: {reason}), Consensus: {consensus}/{opp.get('available_llm_count', 3)}"
                    )
                    ...
                    self.logger.info(
                        ...
                        f"(market price execution), Consensus: {consensus}/{opp.get('available_llm_count', 3)}"
                    )
                else:
                    ...
                    self.logger.info(
                        ...
                        f"(current: {current_price}, diff: {entry_diff_pips:.1f} pips), "
                        f"Consensus: {consensus}/{opp.get('available_llm_count', 3)}"
                    )
    ...
    def _maybe_reset_run_count_and_open_trade(self, opp: Dict, market_state: Dict, opp_id: Optional[str] = None):
        ...
        """
        # cursor6 §5.3: skip re-place if we recently cancelled a stale order at same (pair, direction, entry)
        pair = (opp.get("pair") or "").strip().replace("_", "/").upper()
        ...
        entry = opp.get("entry") or opp.get("current_price") or 0.0
        now_ts = time.time()
        # Purge expired entries
        expired = [k for k, (_, ts) in self._stale_cancel_record.items() if now_ts - ts >= self._stale_cancel_cooldown_seconds]
        for k in expired:
            del self._stale_cancel_record[k]
        key = (pair, direction_norm)
        if key in self._stale_cancel_record and entry > 0:
            entry_cancelled, ts = self._stale_cancel_record[key]
            if now_ts - ts < self._stale_cancel_cooldown_seconds:
                pip_size = 0.01 if "JPY" in pair else 0.0001
                entry_diff_pips = abs(entry - entry_cancelled) / pip_size
                replace_min_pips = float(os.environ.get("REPLACE_ENTRY_MIN_PIPS", "5"))
                if entry_diff_pips < replace_min_pips:
                    self.logger.info(
                        f"⏭️ Skipping re-place after stale cancel for {pair} {direction_norm} @ {entry} "
                        f"(cancelled @ {entry_cancelled}, {entry_diff_pips:.1f} pips < {replace_min_pips} min; cooldown {self._stale_cancel_cooldown_seconds}s)"
                    )
                    return None
        ...
        for trade_id, trade in pending_trades:
            ...
            if current_market_price and current_market_price > 0:
                ...
                if entry_distance_pips > stale_threshold_pips:
                    ...
                    if self.position_manager.executor.cancel_order(
                        ...
                        # cursor6 §5.3: record (pair, direction) and entry so we don't re-place same level within cooldown
                        pair_norm = (pair or "").strip().replace("_", "/").upper()
                        dir_norm = "LONG" if (direction or "").upper() in ("LONG", "BUY") else "SHORT" if (direction or "").upper() in ("SHORT", "SELL") else (direction or "").upper()
                        self._stale_cancel_record[(pair_norm, dir_norm)] = (current_entry, time.time())
                        self.logger.info(
            ...
            if should_replace:
                ...
                if self.position_manager.executor.cancel_order(order_id_to_cancel, f"Replaced: {reason}"):
                    ...
                    # No pending when pair has open: do not place replacement if pair now has an open position
                    if self.position_manager.has_open_position_for_pair(pair):
                        self.logger.warning(
                            f"🔄 Skipping replace for {pair} {direction}: pair has an open position. "
                            f"No pending allowed until it is closed or cancelled."
                        )
    ...
    def _monitor_positions(self, market_state: Dict):
        ...
        # Only close on MACD signal for trades whose exit strategy is MACD_CROSSOVER (improvementplan Fix 2).
        # ATR_TRAILING / BE_TO_TRAILING / STRUCTURE_ATR trades must not be closed by MACD.
        ...
        try:
            while True:
                ...
                if should_run_periodic_sync:
                    ...
                    # Periodic status: log open trades and ages (cursor7 Priority 2.1)
                    try:
                        open_trades = [
                            (tid, trade) 
                            for tid, trade in self.position_manager.active_trades.items()
                        ...
                        if open_trades:
                            ...
                            # Use pytz.UTC instead of timezone.utc for consistency
                            now_utc = datetime.now(pytz.UTC)
                            status_lines = []
                            for tid, trade in open_trades:
                                ...
                                if opened_at:
                                    ...
                                    age_seconds = (now_utc - opened_at_utc).total_seconds()
                                ...
                                status_lines.append(
                                    f"  {trade.pair} {trade.direction} (id={tid[:8]}): "
                                    f"state={trade.state.value}, age={age_str}, PnL={pnl:.1f} pips"
                                )
                            self.logger.info(
                                f"📊 Open trades status ({len(open_trades)} open):\n" + "\n".join(status_lines)
                            )
                        else:
                            self.logger.info("📊 Open trades status: 0 open trades")
                    except Exception as e:
                        self.logger.debug(f"Could not log open trades status: {e}")
                ...
                if self.check_count % 10 == 0:
                    ...
                    if AUTO_TRADER_AVAILABLE:
                        self.logger.info(f"Mode: {self.config.trading_mode.value}")
                        self.logger.info(f"Stop Loss Type: {self.config.stop_loss_type.value}")
                        # cursor6 §5.1: use open count (not total) to match max_trades definition
                        if self.position_manager:
                            summary = self.position_manager.get_active_trades_summary()
                        ...
                        max_trades = self.config.max_open_trades
                        self.logger.info(f"Open Trades: {open_count}/{max_trades}")
                        if pending_count > 0:
