...
def run_daily_learning():
    ...
    logger.info("="*80)
    logger.info("")
    logger.info("LEARNING CYCLE START: Evaluating recommendations and updating LLM weights...")
    ...
    # Capture previous weights (if any) for monitoring changes
    previous_weights = db.get_latest_learning_checkpoint() or {}
    ...
    # Priority 3 (RL Monitoring): explicitly log weight changes vs previous checkpoint
    if previous_weights:
        for llm, new_weight in weights.items():
            old_weight = previous_weights.get(llm, 0.25)
            logger.info(f"WEIGHTS UPDATED: {llm}: {old_weight:.4f} -> {new_weight:.4f}")
    else:
        for llm, new_weight in weights.items():
            logger.info(f"WEIGHTS UPDATED: {llm}: (initial) -> {new_weight:.4f}")
    ...
    logger.info(
        f"LEARNING CYCLE COMPLETE: evaluated={evaluated_count}, "
        f"total_evaluated={total_evaluated}, overall_win_rate={overall_win_rate*100:.1f}%"
    )
    logger.info("")
