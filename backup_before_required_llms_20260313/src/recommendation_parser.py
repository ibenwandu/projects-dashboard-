...
class RecommendationParser:
    ...
    def _parse_text(self, text: str) -> List[Dict]:
        ...
        # ========================================================================
        # PATTERN SET 10: DeepSeek / narrative fallback (consol-recommend4 Phase 2.3, cursor7 enhanced)
        # "**Pair:** EUR/USD", "Pair: EUR/USD", or line "EUR/USD Long" / "EUR/USD - Short"
        # Plus DeepSeek narrative formats: "## Forex Trading Analysis", "### Trade 1:", etc.
        # ========================================================================
        pattern_10a = r'(?:\*\*)?Pair\s*:\s*(?:\*\*)?\s*([A-Z]{3})[/\s-]([A-Z]{3})'
        matches_10a = list(re.finditer(
            rf'{pattern_10a}(.*?)(?=\n\n|\n(?:\*\*)?(?:Pair|Entry|Trade)\s*:|$)',
            text,
            re.IGNORECASE | re.DOTALL
        ))
        # Capture direction in group 3 so _extract_opportunity_from_text gets "Long"/"Short"
        pattern_10b = r'^\s*[-*•]\s*([A-Z]{3})[/\s-]([A-Z]{3})\s*[-:]?\s*((?:Long|Short|BUY|SELL)(?:\s+.*?)?)'
        matches_10b = list(re.finditer(
            rf'{pattern_10b}(?=^\s*[-*•]\s*[A-Z]{{3}}[/\s-]|$)',
            text,
            re.IGNORECASE | re.MULTILINE | re.DOTALL
        ))
        pattern_10c = r'([A-Z]{3})[/\s-]([A-Z]{3})\s+[-–]\s+((?:Long|Short|BUY|SELL)(?:\s+.*?)?)'
        matches_10c = list(re.finditer(
            rf'{pattern_10c}(?=[A-Z]{{3}}[/\s-][A-Z]{{3}}\s+[-–]|\n\n|$)',
            text,
            re.IGNORECASE | re.DOTALL
        ))
        
        # Pattern 10d: DeepSeek narrative - pairs mentioned in analysis sections
        # Matches pairs that appear in narrative text (e.g., "EUR/USD shows weakness" or "I recommend USD/JPY")
        # Look for pairs followed by trading-related keywords within 200 chars
        # FIXED: Simplified to avoid catastrophic backtracking - use simpler pattern without complex lookahead
        # Instead of complex lookahead, match pair + direction keyword directly
        pattern_10d = r'([A-Z]{3})[/\s-]([A-Z]{3})(?:\s+|\s*[-–]\s*)((?:Long|Short|BUY|SELL|Buy|Sell)[^\n]{0,200}?)(?=(?:[A-Z]{3}[/\s-][A-Z]{3}|##|###|\n\n|$))'
        try:
            # Limit text size to prevent excessive processing on very large responses
        ...
        except (re.error, MemoryError, RecursionError) as e:
            logger.warning(f"Pattern 10d regex error: {e}, skipping pattern")
            matches_10d = []
        
        # Pattern 10e: DeepSeek structured trade sections
        # Matches "### Trade 1:", "### **Trade 1:**", "## Trade Recommendation 1:", etc.
        ...
        try:
            ...
            logger.warning(f"Pattern 10e regex error: {e}, skipping pattern")
            matches_10e = []
        
        # Pattern 10f: Fallback - extract pairs from narrative text with direction keywords
        # Looks for "EUR/USD Long", "USD/JPY Short", "GBP/USD BUY", etc. in narrative
        # Group 3 is section text (includes direction keyword and following text)
        # FIXED: Simplified to avoid catastrophic backtracking
        ...
        try:
            ...
            logger.warning(f"Pattern 10f regex error: {e}, skipping pattern")
            matches_10f = []
        
        matches_10 = matches_10a + matches_10b + matches_10c + matches_10d + matches_10e + matches_10f
        
        # Combine all matches
        all_matches = matches_1 + matches_2 + matches_3 + matches_4 + matches_5 + matches_6 + matches_7 + matches_8 + matches_9 + matches_10
        ...
        if len(all_matches) == 0:
            ...
            logger.debug(f"   Pattern matches: P1={len(matches_1)}, P2={len(matches_2)}, P3={len(matches_3)}, P4={len(matches_4)}, P5={len(matches_5)}, P6={len(matches_6)}, P7={len(matches_7)}, P8={len(matches_8)}, P9={len(matches_9)}, P10={len(matches_10)}")
            ...
            # Enhanced logging for DeepSeek (cursor7): Log full response when parser fails
            # Check if this might be DeepSeek output (contains "Forex Trading Analysis" or similar)
            if 'Forex Trading Analysis' in text or 'forex trading' in text.lower():
                ...
                # Try to find any currency pairs in the text
                pair_matches = re.findall(r'([A-Z]{3})[/\s-]([A-Z]{3})', text, re.IGNORECASE)
                if pair_matches:
                    unique_pairs = set([f"{p[0]}/{p[1]}" for p in pair_matches])
                    logger.debug(f"   Found {len(unique_pairs)} unique pairs mentioned in text: {', '.join(sorted(unique_pairs))}")
    ...
    def _extract_opportunity_from_text(self, pair: str, text: str) -> Optional[Dict]:
        ...
        try:
            ...
            entry_patterns = [
                ...
                r'Entry:\s+([0-9]+\.?[0-9]*)',
                # DeepSeek: "Recommended Entry:", "Entry Price:", "Entry:", etc.
                r'Recommended\s+Entry[:\s]+([0-9]+\.?[0-9]*)',
                r'Entry\s+Price[:\s]+([0-9]+\.?[0-9]*)',
            ...
            exit_patterns = [
                ...
                # DeepSeek: "Take Profit 1:", "Target:", "Exit Price:", etc.
            ...
            stop_patterns = [
                ...
                r'SL:\s+([0-9]+\.?[0-9]*)',
                # DeepSeek: "Stop:", "Stop Loss:", "SL:", etc.
                r'Stop[:\s]+([0-9]+\.?[0-9]*)',
                r'Stop\s+Loss[:\s]+([0-9]+\.?[0-9]*)',
            ]
