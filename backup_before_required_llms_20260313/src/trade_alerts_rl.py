        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('''
                INSERT INTO learning_checkpoints (
                    ...
                    synthesis_weight, deepseek_weight, overall_win_rate, notes
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                datetime.utcnow().isoformat(),
                stats['total_recommendations'],
                ...
                weights.get('deepseek', 0.25),
                stats['overall_win_rate'],
                stats.get('notes', '')
            ))
            conn.commit()

    def get_latest_learning_checkpoint(self) -> Optional[Dict[str, float]]:
        """
        Get weights from the most recent learning checkpoint (if any).
        Returns a dict of llm -> weight or None if no checkpoint exists.
        """
        with sqlite3.connect(self.db_path) as conn:
            try:
                cursor = conn.execute(
                    """
                    SELECT chatgpt_weight,
                           gemini_weight,
                           claude_weight,
                           synthesis_weight,
                           COALESCE(deepseek_weight, 0.25) AS deepseek_weight
                ...
                # learning_checkpoints table might not exist yet
                return None
            ...
            chatgpt_w, gemini_w, claude_w, synthesis_w, deepseek_w = row

            return {
                'chatgpt': chatgpt_w if chatgpt_w is not None else 0.25,
                'gemini': gemini_w if gemini_w is not None else 0.25,
                'claude': claude_w if claude_w is not None else 0.25,
                'synthesis': synthesis_w if synthesis_w is not None else 0.25,
                'deepseek': deepseek_w if deepseek_w is not None else 0.25,
            }
