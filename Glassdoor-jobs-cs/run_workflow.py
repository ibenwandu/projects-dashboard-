"""
Fully automated workflow: Glassdoor search -> scrape -> score with AI -> report -> (user approves) -> tailored resumes.

Phases:
  fetch   - Glassdoor search (config/glassdoor_search.json) -> scrape -> save jobs_cache.json + markdown in output/
...
def run_fetch(
    ...
    """Fetch from Glassdoor search, scrape, save cache and markdown. Returns list of JobDetails."""
    config = load_glassdoor_search_config()
    print("Fetching job URLs from Glassdoor search...")
    ...
    if not urls:
        print("No Glassdoor job URLs found. Check config/glassdoor_search.json. Run with --debug to see why.")
        return []
    if max_jobs:
        urls = urls[:max_jobs]
    print("Found", len(urls), "job URL(s). Scraping (Chrome + anti-detection, slower to reduce blocks)...")
    all_jobs = scrape_jobs_with_browser(urls, headless=headless, delay_between=5.0, delay_jitter=3.0, debug=debug)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Glassdoor-jobs workflow: fetch -> score -> report -> resumes")
    parser.add_argument("--phase", choices=["fetch", "score", "report", "resumes", "pdf", "all"], default="all",
                        help="fetch=Glassdoor search+scrape; score+report=score and write report; resumes=tailored resumes + PDFs; pdf=convert .md to PDF only; all=fetch+score+report")
