...
#### 10. Sync and orphan verification (OANDA vs engine/UI)

To verify that open/pending trades are in sync between OANDA, Scalp-Engine, and the UI:

1. **Config API** GET `/trades` (or equivalent) returns the open and pending trades known to the system.
2. Compare with OANDA positions (OANDA dashboard or API) by pair and direction.
3. Any OANDA position with **no matching trade** in GET `/trades` is a **candidate orphan**. Reconcile manually (e.g. cancel on OANDA or add to engine state as needed).

The engine logs "Possible orphan: pair X direction Y on OANDA not in engine state" at WARNING when it detects an OANDA position that it does not track (if optional orphan detection is enabled).
