...
def get_config():
    ...
    try:
        # Priority 1: Return in-memory config if available
        if _in_memory_config is not None:
            ...
            return jsonify(_in_memory_config), 200
...
def update_trade_states():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ Trade states saved to disk: {TRADE_STATES_FILE}")
        except Exception as e:
            logger.warning(f"⚠️ Could not save trade states to disk: {e}")
        
        num_trades = len(trade_states.get('trades', []))
        logger.info(f"✅ Trade states updated in memory - {num_trades} trades (timestamp: {_trade_states_timestamp})")
...
def logs_ingest():
    ...
    try:
        ...
        logger.info(f"Log ingest: {component} -> {filename} ({len(content)} chars)")
        return jsonify({
            'status': 'ok',
            'component': component,
            'filename': filename,
...
def get_log_content(component):
    ...
    try:
        ...
        if not log_files:
            # No log files found - return 404 with helpful message
