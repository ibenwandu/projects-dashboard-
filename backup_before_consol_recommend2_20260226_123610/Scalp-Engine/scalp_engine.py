...
class ScalpEngine:
    ...
    def _check_new_opportunities(self, market_state: Dict):
        ...
        for opp in opportunities:
            try:
                ...
                if already_open:
                    # Log as ERROR since this is a red flag violation
                    self.logger.error(
                ...
                if opp_current_price and opp_current_price > 0 and per_opp_mode not in EXECUTION_MODES_TRIGGER_IS_CROSSOVER:
                    ...
                    if price_diff_pips > max_pips or price_diff_percent > max_percent:
                        ...
                        # Price is slightly stale but within tolerance - log info but allow
                        self.logger.info(
                ...
                if trade:
                    ...
                    # Trade was not opened: may be REJECT (e.g. max_runs exceeded) or OANDA API failure
                    # PositionManager already logs "Opportunity rejected: {reason}" when REJECT
                    self.logger.warning(
                        f"⚠️ Trade not opened for {pair} {direction} "
                        f"(check above for 'Opportunity rejected' e.g. max_runs, or OANDA API)"
                    )
    ...
    def _review_and_replace_pending_trades(self, opportunities: List[Dict], market_state: Optional[Dict] = None):
        ...
        for trade_id, trade in pending_trades:
            ...
            # 1. Better entry price (at least 1 pip difference) - requires matching opportunity
            # 2. Position size changed (config was updated) - doesn't require matching opportunity
