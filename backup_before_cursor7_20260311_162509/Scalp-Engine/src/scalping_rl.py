...
class ScalpingRL:
    ...
    def _init_db(self):
        ...
        # Check if database file already exists (cursor6 §5.6: routine open at DEBUG to reduce log noise)
        db_exists = os.path.exists(str(self.db_path))
        if db_exists:
            db_size = os.path.getsize(str(self.db_path))
            logger.debug(f"📦 Opening existing database: {self.db_path} ({db_size} bytes)")
        else:
