...
class ExecutionModeEnforcer:
    ...
    def reset_run_count(self, opp_id: str):
        ...
        Clears both new-format key (e.g. LLM_EUR/USD_LONG) and legacy key (e.g. EUR/USD_LONG)
        so _get_run_count() no longer finds a stale count via fallback (improvementplan Fix 1).
        """
        deleted = False
