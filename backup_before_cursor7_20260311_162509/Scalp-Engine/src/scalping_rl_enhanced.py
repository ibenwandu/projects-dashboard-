...
# Log "Enhanced RL database initialized" only once per db_path per process (consol-recommend2 Phase 1.1)
_rl_db_init_logged: set = set()
    ...
    def _init_db(self):
        ...
        try:
            ...
            try:
                ...
                conn.commit()
            # Migration: Add deepseek_weight to learning_checkpoints if missing (daily_learning inserts it)
            try:
                c.execute("SELECT deepseek_weight FROM learning_checkpoints LIMIT 1")
            except sqlite3.OperationalError:
                c.execute("ALTER TABLE learning_checkpoints ADD COLUMN deepseek_weight REAL")
                conn.commit()
            # Consensus analysis
            c.execute('''
            ...
            new_columns = [
                ...
                ('trade_id', 'TEXT'),
                ('exit_price', 'REAL'),
                ('pnl_pips', 'REAL'),  # Migration: older DBs had no pnl_pips (caused OperationalError in update_outcome_simulated)
                ('bars_held', 'INTEGER'),
                ('max_favorable_pips', 'REAL'),
                ('max_adverse_pips', 'REAL'),
                ('outcome_timestamp', 'DATETIME'),
            ...
            conn.commit()
            conn.close()
            if self.db_path not in _rl_db_init_logged:
                _rl_db_init_logged.add(self.db_path)
...
def _to_scalar(x):
    """Convert value to Python float; avoid 'ambiguous truth value' when x is a pandas Series."""
...
class OutcomeEvaluator:
    ...
    def evaluate_signal(self, signal: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # Evaluate outcome (scalarize in case signal came from a DataFrame row / pandas Series)
            entry = _to_scalar(signal['entry_price'])
            stop_loss = _to_scalar(signal['stop_loss'])
            take_profit = signal.get('take_profit')
            if take_profit is not None:
                take_profit = _to_scalar(take_profit)
            direction = signal['direction']
            if hasattr(direction, 'iloc'):
                direction = str(direction.iloc[0]) if len(direction) else str(direction)
            elif hasattr(direction, 'item'):
                direction = str(direction.item())
            else:
    ...
    def _check_outcome(
        ...
        Returns:
            Dictionary with outcome, pnl, exit_price, bars_held, max_favorable_pips, max_adverse_pips
        """
        # Ensure scalars (avoid "truth value of a Series is ambiguous" when signal came from DataFrame)
        entry = _to_scalar(entry)
        stop_loss = _to_scalar(stop_loss)
        take_profit = _to_scalar(take_profit) if take_profit is not None else None
        direction = str(direction) if not hasattr(direction, 'iloc') else str(direction.iloc[0]) if len(direction) else str(direction)
        
        if entry is None or stop_loss is None:
            return {
                'outcome': 'MISSED',
                'pnl_pips': 0.0,
                'exit_price': entry or 0.0,
                'bars_held': 0,
        ...
        for i, (timestamp, row) in enumerate(price_data.iterrows()):
            high = _to_scalar(row['High'])
            low = _to_scalar(row['Low'])
            if high is None or low is None:
                continue
            
            if is_long:
                # Track max favorable/adverse (scalars only for max())
        ...
        # If neither TP nor SL hit, check current P&L (scalarize for comparison)
        current_price = price_data.iloc[-1]['Close']
        current_price = _to_scalar(current_price) if current_price is not None else entry
        pip_value = 0.0001 if 'JPY' not in str(entry) else 0.01
