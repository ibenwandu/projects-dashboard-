...
class TradeExecutor:
    ...
    def convert_to_trailing_stop(self, trade_id: str, distance_pips: float, pair: str) -> bool:
        ...
        try:
            ...
            # Optional: cancel existing stop loss order so only one exit order exists (OANDA may reject otherwise)
            try:
                from oandapyV20.endpoints.trades import TradeDetails
                r_td = TradeDetails(accountID=self.account_id, tradeID=trade_id)
                ...
                sl_order_id = (trade_detail.get("stopLossOrder") or {}).get("id")
                if sl_order_id:
                    self.cancel_order(sl_order_id, "Replace with trailing stop")
            except Exception as cancel_e:
                self.logger.debug(f"Could not cancel existing SL order before trailing (non-fatal): {cancel_e}")

            # OANDA v20: create TrailingStopLoss order via OrderCreate (correct endpoint)
            order_body = {
                "order": {
        ...
        if trade.state == TradeState.OPEN:
            ...
            if at_be_or_profit:
                ...
                self.logger.info(
                    ...
                    f"at breakeven/profit, distance={trailing_pips:.1f} pips"
                )
                trade.trailing_distance = trailing_pips
                if self.executor.convert_to_trailing_stop(
                    trade.trade_id, trailing_pips, trade.pair
