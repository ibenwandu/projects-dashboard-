"""Score a job against the candidate profile using Gemini."""
...
@dataclass
class ScoredJob:
    """Job with a match score and reasoning."""
    job: JobDetails
    score: int  # 0-100
    reasoning: str
    jk: str = ""
...
def _job_text(job: JobDetails) -> str:
    """Single block of text describing the job for the prompt."""
    parts = [
        f"Title: {job.title}",
...
def score_job_with_gemini(job: JobDetails, criteria: dict, model: str | None = None) -> ScoredJob:
    """Use Gemini to score how well this job matches the candidate profile. Returns ScoredJob."""
...
def score_jobs(jobs: list[JobDetails], criteria: dict | None = None, model: str | None = None) -> list[ScoredJob]:
    """Score all jobs. Loads criteria from config if not provided."""
    ...
    scored = []
    for job in jobs:
        s = score_job_with_gemini(job, criteria, model=model)
        scored.append(s)
    return scored
