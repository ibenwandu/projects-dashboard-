"""
Convert resume .md files to submission-ready .pdf using the template in config/resume-pdf-template.html.
Uses Playwright (already in use for Indeed) to render HTML and print to PDF.
"""
...
from .config import CONFIG_DIR, RESUMES_DIR

RESUME_PDF_TEMPLATE_PATH = CONFIG_DIR / "resume-pdf-template.html"


def _strip_markdown_fence(text: str) -> str:
    """Remove ```markdown and ``` wrapper if present."""
    text = text.strip()
...
def _contact_to_oneline(soup: BeautifulSoup) -> None:
    """If first h2 is 'Contact Information' followed by ul, replace with single line (template style)."""
...
def _wrap_content_in_template(html_body: str, template_path: Path | None = None) -> str:
    """Wrap rendered resume HTML in the PDF template."""
...
def md_file_to_pdf(
    ...
    """
    Convert a single .md resume file to PDF using the config template.
    Returns the path to the created PDF. PDF is written next to the .md if pdf_path not given.
    """
...
def convert_resume_mds_to_pdfs(
    ...
    """
    Convert all .md files in output/resumes/ to PDF (same base name, .pdf).
    Skips job_links.md. Returns list of created PDF paths.
    """
