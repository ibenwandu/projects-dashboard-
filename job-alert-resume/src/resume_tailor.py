"""Generate customized resumes for approved jobs using Gemini and master resume."""
...
def load_approved_job_ids(path: Path | None = None) -> list[str]:
    """Load approved job IDs from config/approved_jobs.txt (one jk per line)."""
...
def _job_context(job: JobDetails) -> str:
    """Job summary for the resume prompt."""
...
def generate_tailored_resume(job: JobDetails, master_resume: str, model: str | None = None) -> str:
    """Use Gemini to produce a tailored resume for this job from the master resume."""
...
def write_tailored_resumes(
    ...
    """
    Load approved IDs, load jobs from cache, generate tailored resume for each, write to output/resumes/.
    ...
    written = []
    approved_jobs_with_links: list[tuple[JobDetails, str]] = []  # (job, resume_path) for links file
    ...
    if approved_jobs_with_links:
        ...
        # Convert .md resumes to submission-ready .pdf using config template
...
def _write_job_links_file(out_dir: Path, jobs_with_names: list[tuple[JobDetails, str]]) -> Path:
    """Write a single file listing each approved job with its apply link for easy access when applying."""
    lines = [
        "# Job application links",
        "",
        "Use these links to apply for the selected jobs. Each tailored resume is saved in this folder.",
        "",
