...
def generate_report(
    ...
    """
    Write a markdown report: top N jobs first, then full ranked list.
    Saves to reports/<run_id>/report.md and reports/<run_id>/top_jobs.txt (jk per line for easy approval).
    ...
    lines = [
        "# Job Match Report",
        "",
        f"**Top {len(top)} matching jobs** (of {len(scored)} scored).",
        "",
        "---",
        "",
    ]
    ...
    lines.append("")
    lines.append("Example approved_jobs.txt:")
    for s in top[:3]:
        lines.append(f"  {s.jk}")
    lines.append("")
