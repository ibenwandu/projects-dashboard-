...
def _generate_new_sdk(prompt: str, model: str) -> str:
    """Use google-genai (new SDK): from google import genai."""
...
def _generate_legacy_sdk(prompt: str, model: str) -> str:
    """Use google-generativeai (legacy SDK): import google.generativeai as genai."""
    import google.generativeai as genai
    genai.configure(api_key=GEMINI_API_KEY)
    # Legacy SDK often expects model name with or without 'models/' prefix
...
def generate(prompt: str, model: str | None = None, system_instruction: str | None = None) -> str:
    """Generate text with Gemini. Uses google-genai if available, else google-generativeai."""
...
def parse_score_and_reasoning(text: str) -> tuple[int, str]:
    """Parse 'Score: 85' and reasoning from Gemini output. Returns (score 0-100, reasoning)."""
    score = 0
